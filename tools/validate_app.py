...
checks = {
    ...
    "title result copy control present": '>复制</button>' in app and "copyText(title)" in app,
    "product workspace inputs present": all(token in translation for token in ['productRawData', 'productCorrections', '参考标题 / 关键词']),
    "independent product description prompt present": "PW_DESCRIPTION_PROMPT_KEY" in translation and 'id="descriptionPrompt"' in translation,
    "product description generator reuses AI client": "function pwGenerateDescription(" in translation and "callAI(prompt," in translation,
    "product description result is editable and copyable": 'id="descriptionOutput"' in translation and 'id="btnCopyDescription"' in translation,
    "product data feeds title generation": "function pwBuildContext(" in translation and "function pwInstallTitleBridge(" in translation,
    "unified prompt drawer trigger present": 'id = \'btnPrompts\'' in translation and "function pwTogglePromptPanel(" in translation,
    "prompt drawer contains title and description tabs": all(token in translation for token in ['btnPromptTabTitle', 'btnPromptTabDescription', 'promptTitlePane', 'promptDescriptionPane']),
    "legacy inline global prompt section is removed at runtime": "oldPromptSection?.remove();" in translation,
    "prompt drawer supports repeated toggle collapse": "pwTogglePromptPanel()" in translation and "aria-expanded" in translation,
